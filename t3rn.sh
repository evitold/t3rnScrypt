    ...
    case $NODE_TYPE in
        "alchemy-rpc")
            ...
            RPC_ENDPOINTS_JSON=$(echo "$RPC_ENDPOINTS_JSON" | jq \
                ...
                --arg seit "https://evm-rpc-testnet.sei-apis.com.g.alchemy.com/v2/$ALCHEMY_API_KEY" \
                '.arbt += [$arbt] | .bast += [$bast] | .opst += [$opst] | .blst += [$blst] | .unit += [$unit] | .mont += [$mont] | .seit += [$seit]')
            ;;

        "custom-rpc")
            echo -e "${GREEN}Using custom RPC endpoints only...${NC}"
            RPC_ENDPOINTS_JSON=$(echo "$RPC_ENDPOINTS_JSON" | jq 'del(.arbt, .bast, .opst, .blst, .unit, .mont, .seit)')
            ;;
    esac
}
...
DEFAULT_RPC_ENDPOINTS_SEIT=$(echo "$DEFAULT_RPC_ENDPOINTS_JSON" | jq -r '.seit[0]')
...
if [[ "$CUSTOM_RPC" =~ ^[Yy]$ ]]; then
    ...
    declare -A rpc_map=(
        ...
        ["mont"]="Monad testnet"  # Added
        ["seit"]="SEI testnet"  # Added
        ["l2rn"]="L2RN"
    )
...
echo -e "${ORANGE}SEIT = sei-testnet${NC}"
echo -e "${RED}$MSG_L2RN_ALWAYS_ENABLED${NC}"

ENABLED_NETWORKS="l2rn"  # l2rn is now always enabled as base layer
while true; do
    read -p "$(echo -e "${GREEN}Enter networks to enable (comma-separated):\n[ARBT, BAST, BLST, OPST, UNIT, MONT, SEIT] or 'all':${NC} ")" USER_NETWORKS
    if [[ -z "$USER_NETWORKS" || "$USER_NETWORKS" =~ ^[Aa][Ll][Ll]$ ]]; then
        ENABLED_NETWORKS="$ENABLED_NETWORKS,arbitrum-sepolia,base-sepolia,blast-sepolia,optimism-sepolia,unichain-sepolia,monad-testnet,sei-testnet"
        break
    else
        ...
        for network in "${networks[@]}"; do
            case "$network" in
                ...
                MONT)
                    ...
                    ;;
                SEIT)
                    ENABLED_NETWORKS="$ENABLED_NETWORKS,sei-testnet"
                    ;;
                *)
                    echo -e "${RED}Invalid network: $network. Valid options: ARBT, BAST, BLST, OPST, UNIT, MONT, SEIT${NC}"
                    valid=false
                    break
                    ;;
            esac
...
if [[ "$ENABLED_NETWORKS" == *"sei-testnet"* ]]; then
    echo -e "${ORANGE}SEIT:${NC} ${BLUE}$RPC_ENDPOINTS_SEIT${NC}"
fi
